  ...
  useEffect(() => {
    if (navbarState.dropdownState.value === "All" && navbarState.queryString.keyword === '') {
      const fetchData = async () => {
        ...
        try {
          const productsData = await fetch(`http://localhost:4000/api/v1/products`)
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const productsData = await fetch(`http://localhost:4000/api/v1/products?page=${queryString.get('page') || 1}&keyword=${queryString.get('keyword') ? encodedKeyword : ''}${queryString.get('category') && queryString.get('category') !== 'All' ? '&category=' + encodedCategory : ''}`)
        const jsonProductsData = await productsData.json()
        const totalProducts = jsonProductsData.filteredLength
        const resultsPerPage = jsonProductsData.resultsPerPage
        setTotalPages(Math.ceil(totalProducts / resultsPerPage))
